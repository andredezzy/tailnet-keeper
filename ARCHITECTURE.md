...
Mullvad's DNS content blocker does not filter inside the tunnel; it resolves through a different address, `100.64.0.X`, where X is the sum of the enabled lists, one bit each. Those addresses are in `100.64.0.0/10`, the shared range Tailscale assigns its nodes from and installs as a prefix route on its own interface. A `/10` is more specific than the tunnel's default route, so the query leaves for the tailnet and is answered by nobody, and because only name resolution uses it, everything addressed by IP keeps working. A route covering the block is more specific still and returns the query to the tunnel. The keeper places it through whichever interface holds the unscoped default: a scoped default is consulted only by a socket bound to its interface, and Mullvad's resolver binds none, so the unscoped one is the path its query actually takes.

The block is routed rather than the one address the current lists sum to, and it is sized from the `block_` keys the settings declare -- six today, so `100.64.0.0/26`. Picking the exact address would buy precision with a bit table that goes stale the day Mullvad ships another list: the keeper would refuse to route, and name resolution would stop until somebody noticed. Sizing from the declared keys costs 64 addresses of a range holding four million, and the day that list ships the block widens by one bit on its own. Past eight lists the route would start covering tailnet space plausibly in use, and a file claiming that many is one the reader no longer understands, so it degrades instead. The address is computed rather than observed because the query it would be observed from is the one being lost; Mullvad writes its settings as JSON with `null` for unset options, which `plutil` rejects, so the keys are matched as tokens and each must appear exactly once.

The two ranges really do overlap, so a tailnet node inside the block is reported instead of routed over; repairing DNS by taking a node off the tailnet trades one outage for the other. The node check verifies a code signature and is slow, so it runs only when a route is about to change, never on a steady-state pass. That bounds what it can see: a node assigned inside the block after the route has settled is unreachable and the keeper does not look again until the block or the tunnel moves. Checking hourly would close it for the cost of a signature verification per hour, against a collision of 64 addresses in a range of four million. A resolver fault degrades health rather than failing closed, and it is the one degradation the installer and the verifier accept: every bypass route and the PF anchor are exactly as installed while it holds, and rolling an install back over it would keep the machine on the version whose fix it is installing.
