...
Mullvad's DNS content blocker does not filter inside the tunnel; it resolves through a different address, `100.64.0.X`, where X is the sum of the enabled lists. That address is in `100.64.0.0/10`, the shared range Tailscale assigns its nodes from and installs as a prefix route on its own interface. A `/10` is more specific than the tunnel's default route, so the query leaves for the tailnet and is answered by nobody, and because only name resolution uses it, everything addressed by IP keeps working. A host route for the single address is more specific still and returns the query to the tunnel. The keeper places it through whichever interface holds the unscoped default: a scoped default is consulted only by a socket bound to its interface, and Mullvad's resolver binds none, so the unscoped one is the path its query actually takes.

The address is computed, not observed, because the query it would be observed from is the one being lost. Mullvad writes its settings as JSON with `null` for unset options, which `plutil` rejects, so the six booleans are read as tokens: each key must appear exactly once, and a `block_` key the bit table does not name means a list was added whose bit is unknown. Both refuse rather than route a sum that omits it. The two ranges really do overlap, so a resolver address a tailnet peer already holds is reported instead of routed over; repairing DNS by taking a node off the tailnet trades one outage for the other. The peer check verifies a code signature and is slow, so it runs only when a route is about to change, never on a steady-state pass.
